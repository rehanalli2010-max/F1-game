...
export class PhysicsWorld {
  ...
  updateVehicle(vehicle, controls, dt, audioManager = null) {
    ...
    const dragForce = forward.scale(-effectiveDragCoeff * forwardSpeed * Math.abs(forwardSpeed));
    ...
    // Reverse gear handling: only engage when car is fully stopped and brake is held
    if (controls.brake > 0.5 && Math.abs(forwardSpeed) < 0.15) {
      vehicle.isReversing = true;
    } else if (controls.throttle > 0.1 && forwardSpeed > -0.15) {
    ...
    if (vehicle.isReversing) {
      ...
      if (throttle > 0) {
        ...
        // Top speed limit on track
        if (vehicle.isOnTrack && speedKmh > 355) {
          engineForce = 0;
        }

        body.applyForce(forward.scale(engineForce));
      }
      ...
      if (brake > 0 && Math.abs(forwardSpeed) > 0.05) {
        ...
        body.applyForce(forward.scale(-Math.sign(forwardSpeed) * clampedBrakeForce));
      }
    }
    ...
    if (Math.abs(forwardSpeed) > 0.3) {
      ...
      const maxTurnRate = 1.85 * speedFactor;
    ...
    // Lateral slip locked to 0 (100% grip adhesion)
