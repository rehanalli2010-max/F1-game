...
chrome.storage.local.get(function(object) {
    ...
    if (object.migrated !== true) {
        for (var key in object) {
            ...
            else if (key === 'options' && object[key].player_ads) {
                
                object.player_ads = object[key].player_ads;
...
chrome.storage.onChanged.addListener(function(changes) {
    ...
    if (ImprovedTube.isset(changes.options)) {
        let options = changes.options.newValue;
        if(ImprovedTube.isset(options.is_manual) && options.is_manual === true){
            debugger;
            ImprovedTube.storage.player_ads = changes.options.player_ads;
            ImprovedTube.storage.is_manual = true;
...
ImprovedTube.changeArgs = function(args) {
    if (ImprovedTube.isset(args)) {
        ...
        if (
            ...
            ImprovedTube.storage.player_ads === 'subscribed_channels' 
            && (args.player_response || '').indexOf('subscribed=1') === -1
        ) {
